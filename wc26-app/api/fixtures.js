// HYBRID:
//   Base    = worldcup26.ir  (game IDs match local data, reliable finished flags → standings)
//   Overlay = TheSportsDB V2 livescore (paid, fast in-match score updates)
// Live matches get TheSportsDB scores; everything else (results, statuses,
// finished flags) comes from worldcup26.ir.
...
export default async function handler(req, res) {
  ...
  const wc26Headers = { "Content-Type": "application/json" };
  if (wc26Token) wc26Headers["Authorization"] = `Bearer ${wc26Token}`;

  const [wc26Result, sdbResult] = await Promise.allSettled([
    fetch("https://worldcup26.ir/get/games", { headers: wc26Headers, ...timeout() }),
    sdbKey
